...
app.post('/api/financiero/egresos', verificarFinanciero, async (req, res) => {
  const { fecha, punto_servicio_id, punto_servicio_otro, concepto, valor, documento_url, es_costo_financiero, cuenta } = req.body
  ...
  const { data, error } = await supabase.from('egresos')
    .insert({ fecha, punto_servicio_id: psId, punto_servicio_otro: punto_servicio_otro || null, concepto, valor, documento_url: documento_url || null, es_costo_financiero: es_costo_financiero || false, cuenta: cuenta || 'banco', ciudad: req.ciudadFinanciero, registrado_por: id })
    .select().single()
  if (error) return res.status(500).json({ ok: false, mensaje: error.message })
  res.json({ ok: true, data })
})
...
app.put('/api/financiero/egresos/:id', verificarFinanciero, async (req, res) => {
  ...
  const { error } = await supabase.from('egresos')
    .update({ fecha, punto_servicio_id: psId, punto_servicio_otro: punto_servicio_otro || null, concepto, valor, documento_url: documento_url || null, es_costo_financiero: es_costo_financiero || false, cuenta: cuenta || 'banco' })
    .eq('id', req.params.id).eq('ciudad', req.ciudadFinanciero)
  if (error) return res.status(500).json({ ok: false, mensaje: error.message })
  res.json({ ok: true })
})
